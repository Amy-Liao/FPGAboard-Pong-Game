...
module top(LED, display, ssd_control, switch, PS2_DATA, PS2_CLK, clk, rst);
    ...
    game GE0(.LED(LED), .ball_position(ball_position), .next(next), .level(level), .en(en), .slower_clk(slower_clk), .PS2_DATA(PS2_DATA), .PS2_CLK(PS2_CLK), .clk(clk), .rst(rst));

    //ssd
    wire [0:7] display0, display1, display2, display3;
    ssd S0(.display0(display0), .display1(display1), .display2(display2), .display3(display3), .ball_position(ball_position));
